 ...
 */
export function buildChangeTracksRule(config, projectDir, sessionId) {
    function isInScope(file) {
        ...
        if (fs.existsSync(file)) {
            try {
                const head = fs.readFileSync(file, 'utf-8').slice(0, 500);
                const header = parseTrackingHeader(head);
                if (header?.status === 'untracked')
                    return false;
            }
            catch { /* ignore */ }
        }
        // Hook exclusion check
        if (isFileExcludedFromHooks(file, config, projectDir))
            return false;
        return true;
    }
    return {
        ...
        onWrite: async (op) => {
            ...
            if (config.policy.creation_tracking !== 'none' &&
...
//# sourceMappingURL=changetracks-rules.js.map