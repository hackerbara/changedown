...
describe('ensureL2', () => {
  it('promotes L0 insertion to L2 with footnote', () => {
    ...
    expect(result.changeId).toMatch(/^cn-\d+$/);
  ...
  it('returns text unchanged for L2 change (already has footnote)', () => {
    const text = 'Hello {++world++}[^cn-1] more text\n\n[^cn-1]: @alice | 2026-03-09 | ins | proposed\n    reason: test\n';
    const result = ensureL2(text, 6, { author: 'alice', type: 'ins', existingId: 'cn-1' });
    expect(result.promoted).toBe(false);
    expect(result.changeId).toBe('cn-1');
    expect(result.text).toBe(text);
  });

  it('assigns sequential cn-ID based on existing footnotes', () => {
    const text = 'Hello {++a++}[^cn-1] and {++b++} more\n\n[^cn-1]: @alice | 2026-03-09 | ins | proposed\n    reason: test\n';
  ...
  it('assigns cn-1 when no existing footnotes', () => {
