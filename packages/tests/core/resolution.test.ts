...
describe('thread resolution', () => {
  ...
  it('appends resolved line to footnote', () => {
    const edit = computeResolutionEdit(footnote, 'ct-1', { author: '@carol', date: '2026-03-09' });
  ...
  it('returns null when footnote block is not found', () => {
    ...
    const edit = computeResolutionEdit(doc, 'ct-1', { author: '@carol', date: '2026-03-09' });
  ...
  it('inserts after existing approval lines', () => {
    ...
    const edit = computeResolutionEdit(withApproval, 'ct-1', { author: '@carol', date: '2026-03-09' });
    expect(edit).toBeDefined();
  ...
  it('works with multi-footnote documents', () => {
    ...
    const edit = computeResolutionEdit(multi, 'ct-2', { author: '@carol', date: '2026-03-09' });
    expect(edit).toBeDefined();
    const result = applyEdit(multi, edit!);
    // ct-2 block should have the resolved line
    expect(result).toContain('    reason: Removed redundancy\n    resolved: @carol 2026-03-09');
    // ct-1 block should NOT have a resolved line
    const ct1Block = result.split('[^ct-2]:')[0];
...
describe('unresolve thread', () => {
  ...
  it('removes resolved line on unresolve', () => {
    const edit = computeUnresolveEdit(resolvedDoc, 'ct-1');
    expect(edit).toBeDefined();
  ...
  it('returns null when no resolved line exists', () => {
    ...
    const edit = computeUnresolveEdit(noResolved, 'ct-1');
    expect(edit).toBeNull();
  });

  it('returns null when footnote block is not found', () => {
    const doc = 'Hello {++world++}[^ct-1]\n';
    const edit = computeUnresolveEdit(doc, 'ct-1');
    expect(edit).toBeNull();
  });
  ...
  it('only removes resolved line from correct footnote', () => {
    ...
    const edit = computeUnresolveEdit(multi, 'ct-1');
    expect(edit).toBeDefined();
    const result = applyEdit(multi, edit!);
    // ct-1 resolved line gone
    const ct1Section = result.split('[^ct-2]:')[0];
    expect(ct1Section).not.toContain('resolved:');
    // ct-2 resolved line preserved
    const ct2Section = result.split('[^ct-2]:')[1];
