...
describe('applyReview — withdraw', () => {
  it('appends withdrew: line to footnote', () => {
    const result = applyReview(baseDoc, 'cn-1', 'withdraw', 'Addressed in cn-5', 'carol');
    expect('error' in result).toBe(false);
    if ('updatedContent' in result) {
      expect(result.updatedContent).toContain('withdrew: @carol');
      expect(result.updatedContent).toContain('"Addressed in cn-5"');
    }
  });

  it('does NOT change status', () => {
    const result = applyReview(baseDoc, 'cn-1', 'withdraw', 'Reason', 'carol');
    if ('updatedContent' in result) {
  ...
  it('does NOT cascade to children', () => {
    const docWithChildren = baseDoc + '\n\n' +
      '[^cn-1.1]: @alice | 2026-03-15 | ins | proposed\n' +
      '    4:b2 child';
    const result = applyReview(docWithChildren, 'cn-1', 'withdraw', 'Reason', 'carol');
    if ('updatedContent' in result) {
      expect(result.result.cascaded_children).toBeUndefined();
      // Child should still be proposed
      expect(result.updatedContent).toMatch(/\[.cn-1\.1\].*\| proposed/);
    }
  });
  ...
  it('accepts withdraw without prior request-changes (forgiving input)', () => {
    ...
    const result = applyReview(cleanDoc, 'cn-1', 'withdraw', 'No longer needed', 'bob');
    expect('error' in result).toBe(false);
    if ('updatedContent' in result) {
