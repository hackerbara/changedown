...
import { scrubBackward, scrubForward, type ActiveOperation } from '@changedown/core/internals';
import { FootnoteNativeParser } from '@changedown/core/internals';
...
describe('scrubBackward', () => {
  it('un-applies a single insertion to reach body_0', () => {
    ...
    expect(result.positions.get('cn-1')).toBeDefined();
    expect(result.positions.get('cn-1')!.offset).toBe(4);
  });

  it('un-applies a single deletion to reach body_0', () => {
    const body = 'The lazy dog';
    const ops: ActiveOperation[] = [{
      id: 'cn-1', type: 'deletion',
  ...
  it('resolves edit-over-edit by un-applying in reverse log order', () => {
    ...
    expect(result.positions.get('cn-1')).toBeDefined();
    expect(result.positions.get('cn-1')!.resolved).toBe(true);
  });

  it('un-applies a substitution to reach body_0', () => {
    const body = 'The slow brown fox';
    const ops: ActiveOperation[] = [{
      id: 'cn-1', type: 'substitution',
  ...
  it('skips rejected operations', () => {
    ...
    const result = scrubBackward(body, ops);
    expect(result.body0).toBe('The lazy dog');
    expect(result.positions.has('cn-1')).toBe(false);
  });
  ...
  it('marks an operation as unresolved when its text cannot be found', () => {
    ...
    const result = scrubBackward(body, ops);
    expect(result.positions.get('cn-1')!.resolved).toBe(false);
...
describe('scrubForward', () => {
  it('computes fresh anchors for a single insertion', () => {
    ...
    expect(forward.anchors.get('cn-1')).toBeDefined();
    expect(forward.anchors.get('cn-1')).toContain('{++very ++}');
  ...
  it('detects full consumption when a later deletion removes an earlier insertion', () => {
    ...
    expect(forward.consumption.get('cn-1')).toEqual({
      consumedBy: 'cn-2',
      type: 'full',
    });
  });
  ...
  it('detects partial consumption when a substitution overlaps an insertion', () => {
    ...
    expect(forward.consumption.get('cn-1')).toBeDefined();
    expect(forward.consumption.get('cn-1')!.consumedBy).toBe('cn-2');
  });
  ...
  it('reports no consumption when operations target different regions', () => {
    ...
    expect(forward.consumption.has('cn-1')).toBe(false);
    expect(forward.consumption.has('cn-2')).toBe(false);
  });

  it('consumed op gets footnoteLineRange as its range and consumptionType propagated', async () => {
    // L3 document where cn-2 (deletion) fully consumes cn-1 (insertion) at same location.
