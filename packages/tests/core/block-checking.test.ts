...
import { applyReview, DEFAULT_CONFIG } from '@changedown/core';
...
describe('block checking on acceptance', () => {
  it('prevents acceptance when unresolved block exists', () => {
    ...
    const result = applyReview(doc, 'cn-1', 'approve', 'Looks good', 'bob', DEFAULT_CONFIG);
  ...
  it('allows acceptance when block is resolved via withdrew', () => {
    ...
    const result = applyReview(doc, 'cn-1', 'approve', 'Good now', 'bob', DEFAULT_CONFIG);
  ...
  it('allows acceptance when block is resolved via approve from blocker', () => {
    ...
    const result = applyReview(doc, 'cn-1', 'approve', 'Agreed', 'bob', DEFAULT_CONFIG);
    expect('error' in result).toBe(false);
  });
  ...
  it('allows acceptance when no blocks exist', () => {
    ...
    const result = applyReview(doc, 'cn-1', 'approve', 'Fine', 'bob', DEFAULT_CONFIG);
    expect('error' in result).toBe(false);
  });
  ...
  it('checks may_review gate', () => {
    ...
    const result = applyReview(doc, 'cn-1', 'approve', 'OK', 'ai:claude', config);
    expect('error' in result).toBe(true);
    if ('error' in result) {
  ...
  it('checks self_acceptance gate', () => {
    ...
    const result = applyReview(doc, 'cn-1', 'approve', 'OK', 'alice', config);
    expect('error' in result).toBe(true);
    if ('error' in result) {
