...
import { initHashline, parseForFormat, resolve } from '@changetracks/core/internals';
...
describe('ChangeNode enrichment from FootnoteNativeParser', () => {
  it('populates footnoteLineRange for each change', () => {
    ...
    const change = changes.find(c => c.id === 'ct-1');
  ...
  it('populates replyCount from thread replies', () => {
    ...
    const doc = parseForFormat(text);
    const change = doc.getChanges().find(c => c.id === 'ct-1');
...
describe('resolve() and parseForFormat() parity', () => {
  it('resolve returns same coherence rate as parseForFormat for well-formed L3', async () => {
    ...
    // Both should resolve ct-1 successfully
    const resolvedChange = resolved.changes.find(c => c.id === 'ct-1');
    const parsedChange = parsed.getChanges().find(c => c.id === 'ct-1');
  ...
  it('resolve and parseForFormat agree on consumedBy for edit-over-edit', async () => {
    ...
    const resolved = resolve(text);
    const parsed = parseForFormat(text);
    const resolvedCt1 = resolved.changes.find(c => c.id === 'ct-1');
    const parsedCt1 = parsed.getChanges().find(c => c.id === 'ct-1');
    // resolve() should still show ct-1 as consumed by ct-2
    expect(resolvedCt1?.consumedBy).toBe('ct-2');
    // parseForFormat via scrub replay should also set consumedBy
    expect(parsedCt1?.consumedBy).toBe('ct-2');
  });
});
...
describe('ChangeNode resolution fields from scrub integration', () => {
  it('populates resolutionPath as replay for scrub-resolved changes', () => {
    // Edit-over-edit: ct-1 inserts "very " (ambiguous — appears twice in body),
    // ct-2 substitutes "lazy"→"sleepy" on same line. Hash ff is wrong for both.
    // ct-1 can't resolve via context (body changed by ct-2) or text search
