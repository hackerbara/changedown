...
import { computeAmendEdits, initHashline } from '@changedown/core';
...
describe('computeAmendEdits', () => {
  ...
  it('rewrites insertion inline text', () => {
    const result = computeAmendEdits(baseDoc, 'cn-1', {
      newText: 'world',
      reason: 'Fixed typo',
      author: '@alice',
    });
  ...
  it('preserves footnote ref in rewritten markup', () => {
    ...
    expect(result.isError).toBe(false);
    if (!result.isError) {
      expect(result.text).toContain('{++world++}[^cn-1]');
    }
  });
  ...
  it('rewrites substitution modified text', () => {
    const subDoc = [
      'Hello {~~old~>nwe~~}[^cn-1] more text',
      '',
      '[^cn-1]: @alice | 2026-03-09 | sub | proposed',
      '    reason: Fixed word',
      '',
    ].join('\n');
    const result = computeAmendEdits(subDoc, 'cn-1', {
      newText: 'new',
  ...
  it('supports substitution scope expansion with oldText', () => {
    const subDoc = [
      'prefix Hello {~~old~>new~~}[^cn-1] suffix more text',
      '',
      '[^cn-1]: @alice | 2026-03-09 | sub | proposed',
      '    reason: Changed word',
      '',
    ...
    if (!result.isError) {
      ...
      expect(result.text).toContain('prefix {~~Hello old suffix~>replacement~~}[^cn-1] more text');
    }
  });

  it('rewrites comment text', () => {
    const commentDoc = [
      'Hello {>>old note<<}[^cn-1] more text',
      '',
      '[^cn-1]: @alice | 2026-03-09 | com | proposed',
      '    reason: Comment',
      '',
    ].join('\n');
    const result = computeAmendEdits(commentDoc, 'cn-1', {
  ...
  it('rejects amend on non-proposed change (accepted)', () => {
    ...
    const result = computeAmendEdits(acceptedDoc, 'cn-1', {
      newText: 'world',
      reason: 'Fix',
      author: '@alice',
    });
  ...
  it('rejects amend on non-proposed change (rejected)', () => {
    ...
    const result = computeAmendEdits(rejectedDoc, 'cn-1', {
      newText: 'world',
      reason: 'Fix',
      author: '@alice',
    });
  ...
  it('rejects newText on deletion changes', () => {
    const delDoc = [
      'Hello {--removed--}[^cn-1] more text',
      '',
      '[^cn-1]: @alice | 2026-03-09 | del | proposed',
      '    reason: Removed text',
      '',
    ].join('\n');
    const result = computeAmendEdits(delDoc, 'cn-1', {
      newText: 'something',
      reason: 'Fix',
      author: '@alice',
    });
  ...
  it('returns error for nonexistent change ID', () => {
    const result = computeAmendEdits(baseDoc, 'cn-99', {
      newText: 'world',
      reason: 'Fix',
      author: '@alice',
    });
  ...
  it('truncates previous text longer than 100 characters', () => {
    ...
    const longDoc = [
      `Hello {++${longText}++}[^cn-1] more text`,
      '',
      '[^cn-1]: @alice | 2026-03-09 | ins | proposed',
      '    reason: Added long text',
      '',
    ].join('\n');
    const result = computeAmendEdits(longDoc, 'cn-1', {
      newText: 'short',
      reason: 'Shorten',
      author: '@alice',
    });
...
describe('computeAmendEdits on L3 (footnote-native)', () => {
  ...
  it('finds the change and produces valid edits for L3 format', () => {
    const result = computeAmendEdits(l3Doc, 'cn-1', {
      newText: 'world ',
      reason: 'Fixed typo',
      author: '@alice',
    });
