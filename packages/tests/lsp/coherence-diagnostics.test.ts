...
import { createDiagnostics } from '@changetracks/lsp-server/internals';
import { DiagnosticSeverity } from 'vscode-languageserver/node';
import { ChangeType, ChangeStatus } from '@changetracks/core';
import type { ChangeNode, UnresolvedDiagnostic } from '@changetracks/core';
...
describe('diagnostics with unanchored changes', () => {
  ...
  it('emits Warning diagnostic for unresolved L2+ changes', () => {
    ...
    expect(result[0].code).toBe('ct-5');
  });
  ...
  it('handles unresolvedDiagnostics entries with no matching change gracefully', () => {
    ...
    const unresolvedDiagnostics: UnresolvedDiagnostic[] = [{
      changeId: 'ct-999', // no matching change
      expectedText: 'ghost',
      actualLineContent: '',
      attemptedPaths: ['hash'],
    }];
    const result = createDiagnostics(changes, text, unresolvedDiagnostics);
    // ct-1 should still get a Warning (fallback message, no detail from map)
    const warning = result.find(d => d.code === 'ct-1');
