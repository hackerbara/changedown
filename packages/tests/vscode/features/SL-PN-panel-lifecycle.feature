...
Feature: SL-PN — Review panel lifecycle in running VS Code
  ...
  Scenario: Cards show correct type and status badges
    Then the card for ct-1 shows type "insertion" and status "proposed"
    And the card for ct-2 shows type "substitution" and status "proposed"
    And the card for ct-3 shows type "insertion" and status "accepted"
    And the card for ct-4 shows type "deletion" and status "rejected"
    And the card for ct-5 shows type "highlight" and status "proposed"

  @SL-PN-03
  Scenario: Card click navigates to change location
    When I click the card for ct-3
  ...
  Scenario: Accept from panel button
    When I click Accept on the card for ct-1
  ...
  Scenario: Filter to proposed only
    ...
    And the visible cards are ct-1, ct-2, ct-5
