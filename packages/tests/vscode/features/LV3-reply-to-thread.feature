...
Feature: LV3 — Reply to thread
  ...
  Replying to a cn-ID thread appends a discussion line to the footnote.
  ...
  Scenario: Reply appends to existing footnote
    Given a reply document with text:
      ...
      """
    When I reply to cn-1 with "Looks good, minor typo on line 3"
    Then the reply result footnote for cn-1 contains "@bob 2026-03-09"
    And the reply result footnote for cn-1 contains "Looks good, minor typo on line 3"
  ...
  Scenario: LV3-03 Multi-reply chain (3 replies by different authors)
    Given a reply document with text:
      ...
      [^cn-1]: @alice | 2026-03-09 | insertion | proposed
          reason: Added for clarity
      """
    When I reply to cn-1 with "First comment"
    And I reply again to cn-1 with "Second comment" as "@carol"
    And I reply again to cn-1 with "Third comment" as "@dave"
    Then the reply result footnote for cn-1 contains "First comment"
    And the reply result footnote for cn-1 contains "Second comment"
    And the reply result footnote for cn-1 contains "Third comment"
  ...
  Scenario: LV3-04 Reply to already-accepted change (post-decision discussion)
    Given a reply document with text:
      ...
      """
    When I reply to cn-1 with "Post-acceptance note: verify edge case"
    Then the reply result footnote for cn-1 contains "Post-acceptance note: verify edge case"
  ...
  Scenario: LV3-05 Reply with multi-line text
    Given a reply document with text:
      ...
      [^cn-1]: @alice | 2026-03-09 | insertion | proposed
          reason: Added for clarity
      """
    When I reply to cn-1 with "Line one of the reply\nLine two continues here"
    Then the reply result footnote for cn-1 contains "Line one of the reply"
    And the reply result footnote for cn-1 contains "Line two continues here"
