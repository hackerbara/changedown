...
Feature: SL-XS — Cross-surface sync in running VS Code
  ...
  Scenario: Accept from CodeLens updates panel and thread
    ...
    When I click Accept on the CodeLens for ct-1
    And I select QuickPick item "Accept"
    Then the panel card for ct-1 shows status "accepted"
    And the thread for ct-1 footnote contains "approved:"
    And the CodeLens for ct-1 no longer shows Accept | Reject
  ...
  Scenario: Reply from peek updates CodeLens indicator and panel count
    ...
    When I position cursor inside ct-1 insertion
    And I open the comment thread for ct-1
    And I type "Additional feedback" in the reply box
    And I click the Reply button
    Then the CodeLens for ct-1 shows updated discussion count
    And the panel card for ct-1 shows updated reply count
    And I capture evidence screenshot "cross-reply-after"

  @SL-XS-03
  Scenario: Resolve updates CodeLens and panel state
    When I execute "changetracks.resolveThread" on ct-2
    Then the CodeLens for ct-2 does not show discussion indicator
    And the panel card for ct-2 shows resolved state
    And the thread for ct-2 has state "resolved"
