...
Feature: Cross-Surface Coherence - Agent MCP and VS Code
  ...
  Scenario: Agent batch output with dotted IDs parsed correctly
    Given the input text is:
      """
      {~~alpha~>ALPHA~~}[^ct-1.1] and {~~gamma~>GAMMA~~}[^ct-1.2]

      [^ct-1]: @ai:claude | 2026-02-10 | group | proposed
          Batch capitalize
      [^ct-1.1]: @ai:claude | 2026-02-10 | sub | proposed
          Cap first
      [^ct-1.2]: @ai:claude | 2026-02-10 | sub | proposed
          Cap third
      """
    When I parse the text
    ...
    And change 1 id starts with "ct-1."
    And change 2 id starts with "ct-1."
    And change 1 is a substitution
  ...
  Scenario: Human adds footnote manually and it parses correctly
    Given the input text is:
      ...
      [^ct-1]: @human:alice | 2026-02-10 | sub | proposed
  ...
  Scenario: Document with changes from multiple agents and humans
    Given the input text is:
      ...
      Auth: {~~keys~>OAuth2~~}[^ct-2].
      {++Rate limit: 1000/min++}[^ct-3]

      [^ct-1]: @ai:claude | 2026-02-10 | sub | proposed
      [^ct-2]: @human:alice | 2026-02-10 | sub | accepted
          approved: @human:bob 2026-02-10
      [^ct-3]: @ai:drafter | 2026-02-10 | ins | proposed
      """
    When I parse the text
    Then the parser finds 3 changes
    And change with id "ct-1" has author "@ai:claude"
    And change with id "ct-2" has author "@human:alice"
    And change with id "ct-3" has author "@ai:drafter"
    And change with id "ct-2" has status "accepted"
    And change with id "ct-1" has status "proposed"
