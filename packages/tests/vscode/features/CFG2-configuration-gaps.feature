...
Feature: CFG2 — Configuration Setting Coverage Gaps
  ...
  Each targets a declared changedown.* setting from package.json
  ...
  Scenario: CFG2-01 editBoundary.pauseThresholdMs is read directly from config
    Given the setting "changedown.editBoundary.pauseThresholdMs" is 5000
  ...
  Scenario: CFG2-02 defaultViewMode sets initial view on file open
    Given the setting "changedown.defaultViewMode" is "simple"
  ...
  Scenario: CFG2-03 author setting appears in tracked changes
    Given the setting "changedown.author" is "alice"
  ...
  Scenario: CFG2-04 confirmBulkThreshold triggers confirmation dialog
    Given the setting "changedown.confirmBulkThreshold" is 2
    And a document with 3 tracked changes
    When I execute "ChangeDown: Accept All Changes"
  ...
  Scenario: CFG2-06 decorationStyle "background" uses background tinting
    Given the setting "changedown.decorationStyle" is "background"
  ...
  Scenario: CFG2-07 clickToShowComments controls click-to-expand behavior
    Given the setting "changedown.clickToShowComments" is true
  ...
  Scenario: CFG2-08 preferGutter prompts to disable Git SCM
    Given the setting "changedown.preferGutter" is true
  ...
  Scenario: CFG2-09 commentInsertAuthor false omits author from comments
    Given the setting "changedown.commentInsertAuthor" is false
    And the setting "changedown.author" is "alice"
  ...
  Scenario: CFG2-10 preview.showFootnotes false hides footnote panel
    Given the setting "changedown.preview.showFootnotes" is false
  ...
  Scenario: CFG2-11 preview.metadataDetail "projected" shows all metadata inline
    Given the setting "changedown.preview.metadataDetail" is "projected"
  ...
  Scenario: CFG2-12 showWalkthroughOnStartup "never" suppresses walkthrough
    Given the setting "changedown.showWalkthroughOnStartup" is "never"
