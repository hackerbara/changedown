...
import { buildAnnotationCards, renderMarkdownToHtml, buildLoadingHtml, buildErrorHtml, buildChoiceHtml, buildPreviewHtml } from 'changetracks-vscode/internals';
import type { AnnotationCard, ImportStats } from 'changetracks-vscode/internals';
...
Given('docx preview source text {string}', function (this: ChangeTracksWorld, text: string) {
    this.docxPreviewSource = text;
});

// -- When steps -----------------------------------------------------------

When('I extract comment pairs', function (this: ChangeTracksWorld) {
...
Then('there is {int} comment pair(s)', function (this: ChangeTracksWorld, count: number) {
...
Then('there are {int} comment pair(s)', function (this: ChangeTracksWorld, count: number) {
    assert.ok(this.annotationCards !== undefined, 'No annotation cards — run "I extract comment pairs" first');
    assert.strictEqual(
        this.annotationCards!.length,
        count,
...
Then('comment pair {int} has highlightText {string}', function (this: ChangeTracksWorld, index: number, expected: string) {
...
Then('comment pair {int} has commentText {string}', function (this: ChangeTracksWorld, index: number, expected: string) {
    assert.ok(this.annotationCards !== undefined, 'No annotation cards');
    const card = this.annotationCards![index];
    assert.ok(card, `Annotation card ${index} does not exist (have ${this.annotationCards!.length})`);
    assert.strictEqual(
...
Then('comment pair {int} has no highlightText', function (this: ChangeTracksWorld, index: number) {
    assert.ok(this.annotationCards !== undefined, 'No annotation cards');
    const card = this.annotationCards![index];
    assert.ok(card, `Annotation card ${index} does not exist (have ${this.annotationCards!.length})`);
...
Then('comment pair {int} has a pairId matching {string}', function (this: ChangeTracksWorld, index: number, prefix: string) {
    assert.ok(this.annotationCards !== undefined, 'No annotation cards');
    const card = this.annotationCards![index];
    assert.ok(card, `Annotation card ${index} does not exist (have ${this.annotationCards!.length})`);
    assert.ok(
...
Given('docx preview markdown {string}', function (this: ChangeTracksWorld, text: string) {
    this.docxPreviewMarkdown = text.replace(/\\n/g, '\n');
});

// -- When steps: markdown rendering ---------------------------------------

When('I render markdown to preview HTML', function (this: ChangeTracksWorld) {
...
Then('the docx preview HTML contains {string}', function (this: ChangeTracksWorld, expected: string) {
...
Given('a docx file named {string}', function (this: ChangeTracksWorld, name: string) {
    this.docxFileName = name;
});

Given('an existing markdown file {string}', function (this: ChangeTracksWorld, name: string) {
    this.docxExistingMdName = name;
});

Given('a docx preview with body {string}', function (this: ChangeTracksWorld, html: string) {
...
Given('a comment pair with pairId {string} and text {string} by {string}', function (this: ChangeTracksWorld, id: string, text: string, author: string) {
...
Given('import stats of {int} insertions, {int} deletion, {int} substitutions by {string}', function (this: ChangeTracksWorld, ins: number, del: number, sub: number, author: string) {
...
When('I build loading HTML', function (this: ChangeTracksWorld) {
    assert.ok(this.docxFileName, 'No docx file name set');
    this.webviewHtml = buildLoadingHtml(this.docxFileName!);
});

When('I build error HTML with message {string}', function (this: ChangeTracksWorld, msg: string) {
    assert.ok(this.docxFileName, 'No docx file name set');
    this.webviewHtml = buildErrorHtml(this.docxFileName!, msg);
});

When('I build choice HTML', function (this: ChangeTracksWorld) {
    assert.ok(this.docxFileName, 'No docx file name set');
...
When('I build preview HTML', function (this: ChangeTracksWorld) {
    ...
    this.webviewHtml = buildPreviewHtml({
        ...
        currentViewMode: 'allMarkup',
    });
});

When('I build preview HTML with view mode {string}', function (this: ChangeTracksWorld, viewMode: string) {
    assert.ok(this.previewBodyHtml !== undefined, 'No preview body HTML set');
    this.webviewHtml = buildPreviewHtml({
        fileName: this.docxFileName ?? 'test.docx',
        bodyHtml: this.previewBodyHtml!,
...
When('I render markdown to preview HTML in {string} mode', function (this: ChangeTracksWorld, viewMode: string) {
    assert.ok(this.docxPreviewMarkdown !== undefined, 'No markdown set');
    this.docxPreviewHtml = renderMarkdownToHtml(this.docxPreviewMarkdown!, false, viewMode);
});

Then('the webview HTML contains {string}', function (this: ChangeTracksWorld, expected: string) {
...
Then('the docx preview HTML does not contain {string}', function (this: ChangeTracksWorld, unexpected: string) {
    assert.ok(this.docxPreviewHtml !== undefined, 'No preview HTML generated');
