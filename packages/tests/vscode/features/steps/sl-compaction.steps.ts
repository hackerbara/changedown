 ...
 */
async function hasAnyNotificationToast(world: ChangeDownWorld): Promise<boolean> {
    assert.ok(world.page, 'Page not available');
...
Then(
    ...
    async function (this: ChangeDownWorld, unexpected: string) {
        ...
        await executeCommandViaBridge(this.page, 'changedown._testWaitForChanges');
...
Then(
    ...
    { timeout: 12000 },
    async function (this: ChangeDownWorld, keyword: string) {
        assert.ok(this.page, 'Page not available');
...
When(
    'I confirm {string}',
    { timeout: 10000 },
    async function (this: ChangeDownWorld, buttonLabel: string) {
        assert.ok(this.page, 'Page not available');
