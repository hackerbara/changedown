...
Feature: LV6 — Supersede change
  ...
  Scenario: Supersede rejects original and creates replacement
    Given a supersede document with text:
      """
      Hello {~~wrold~>world~~}[^ct-1]

      [^ct-1]: @alice | 2026-03-09 | substitution | proposed
      """
    And supersede author is "@bob"
    When I supersede ct-1 with "universe" and reason "Better word choice"
    Then the supersede result footnote status for ct-1 is "rejected"
    And a new change exists in the supersede result with text "universe"
    And the new change supersede result footnote contains "supersedes: ct-1"
  ...
  Scenario: LV6-02 Cannot supersede own change
    ...
    When I try to supersede ct-1 with "universe" and reason "Self-supersede"
  ...
  Scenario: LV6-03 Cannot supersede already-accepted change
    Given a supersede document with text:
      ...
      [^ct-1]: @alice | 2026-03-09 | substitution | accepted
          approved: @carol 2026-03-09 "Looks good"
      """
    And supersede author is "@bob"
    When I try to supersede ct-1 with "universe" and reason "Better choice"
  ...
  Scenario: LV6-04 Cannot supersede already-rejected change
    Given a supersede document with text:
      ...
      [^ct-1]: @alice | 2026-03-09 | substitution | rejected
  ...
  Scenario: LV6-05 Superseded-by back-link written to original footnote
    ...
    And supersede author is "@bob"
    When I supersede ct-1 with "universe" and reason "Better word choice"
    Then the supersede result footnote for ct-1 contains "superseded-by:"
  ...
  Scenario: LV6-06 Supersede with reason records rejection reason on original
    Given a supersede document with text:
      ...
      """
    And supersede author is "@bob"
    When I supersede ct-1 with "universe" and reason "Outdated terminology"
    Then the supersede result footnote for ct-1 contains "Outdated terminology"
  ...
  Scenario: LV6-07 Supersede on insertion type
    Given a supersede document with text:
      ...
      """
    And supersede author is "@bob"
    When I supersede ct-1 with "universe" and reason "Different word"
    Then the supersede result footnote status for ct-1 is "rejected"
    And a new change exists in the supersede result with text "universe"
