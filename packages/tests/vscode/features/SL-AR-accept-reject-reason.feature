...
Feature: SL-AR — Accept/reject with reason in running VS Code
  ...
  Scenario: Quick accept (no reason) via QuickPick when reason optional
    ...
    When I position cursor inside the cn-1 insertion
    ...
    Then the document footnote for cn-1 contains "accepted"
  ...
  Scenario: Accept with reason via QuickPick
    Given reasonRequired is set to false
    When I position cursor inside the cn-2 substitution
    And I execute "changedown.acceptChange"
    ...
    Then the document footnote for cn-2 contains "approved:"
    And the document footnote for cn-2 contains "Clear improvement"
  ...
  Scenario: Request changes from accept QuickPick
    ...
    Then the document footnote for cn-1 contains "request-changes:"
    And the document footnote for cn-1 contains "Too vague, add specifics"
    And the footnote status for cn-1 is still "proposed"
  ...
  Scenario: Accept with mandatory reason (InputBox, no QuickPick)
    ...
    Then the document footnote for cn-1 contains "approved:"
    And the document footnote for cn-1 contains "Approved after review"
  ...
  Scenario: Mandatory reason blocks empty submission
    ...
    Then the footnote status for cn-1 is still "proposed"

  @SL-AR-06
  Scenario: Reject with reason via QuickPick
    Given reasonRequired is set to false
    When I position cursor inside the cn-3 highlight
    And I execute "changedown.rejectChange"
    And I select QuickPick item "Reject with reason..."
    Then an InputBox appears
    When I type "Not relevant to this section" in the InputBox
    Then the document footnote for cn-3 contains "rejected:"
    And the document footnote for cn-3 contains "Not relevant to this section"
