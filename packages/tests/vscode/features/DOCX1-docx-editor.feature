...
Feature: DOCX1 — DOCX custom editor
  ...
  Scenario: DOCX1-01 Opening .docx shows custom editor panel
    Given I open "stress-test-via-changedown-output.docx" in VS Code
  ...
  Scenario: DOCX1-02 Import button creates -changedown.md file
    Given I open "stress-test-via-changedown-output.docx" in VS Code
    And I wait 2000 milliseconds
    # Import button click would create stress-test-via-changedown-output-changedown.md
    Then the status bar shows "docx"

  # ── Filename convention ──────────────────────────────────────

  Scenario: DOCX1-03 Generated filename follows -changedown.md convention
    Given I open "stress-test-via-changedown-output.docx" in VS Code
    And I wait 2000 milliseconds
    Then the status bar shows "docx"
    # Expected output: stress-test-via-changedown-output-changedown.md
  ...
  Scenario: DOCX1-04 Export to DOCX command available on markdown
    ...
    When I execute "ChangeDown: Export to DOCX"
  ...
  Scenario: DOCX1-05 Export command on non-markdown shows warning
    ...
    And I execute "ChangeDown: Export to DOCX"
    And I wait 1000 milliseconds
  ...
  Scenario: DOCX1-06 Open existing button appears when -changedown.md exists
    Given I open "stress-test-via-changedown-output.docx" in VS Code
    And I wait 2000 milliseconds
    Then the status bar shows "docx"
    # If stress-test-via-changedown-output-changedown.md exists, "Open existing" shown
