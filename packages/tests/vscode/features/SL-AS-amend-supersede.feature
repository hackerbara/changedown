...
Feature: SL-AS — Amend and supersede in running VS Code
  ...
  Scenario: Amend blocked for wrong author
    When I position cursor inside the cn-1 insertion "Alice's addition"
    And I execute "changedown.amendChange"
  ...
  Scenario: Amend own change succeeds
    ...
    And the document footnote for cn-1 contains "revised @alice"
    And the document footnote for cn-1 contains "previous:"
    And I capture evidence screenshot "after-amend"

  @SL-AS-03
  Scenario: Supersede another author's proposed change
    When I position cursor inside the cn-2 insertion "Bob's addition"
    And I execute "changedown.supersedeChange"
    ...
    Then the document footnote for cn-2 contains "rejected:"
    And the document footnote for cn-2 contains "superseded-by:"
  ...
  Scenario: Supersede blocked on already-accepted change
    When I position cursor inside the cn-3 insertion "accepted text"
    And I execute "changedown.supersedeChange"
