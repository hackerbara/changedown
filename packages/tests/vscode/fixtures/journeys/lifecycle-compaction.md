...
The configuration system {++loads settings from environment variables with fallback to config files++}[^ct-1] for flexibility.

## Proposed Change (Blocked - Not Yet Decided)

The queue processor {++implements dead-letter routing for failed messages after 3 retries++}[^ct-2] for reliability.

## Change With Unresolved Discussion

The caching strategy {~~invalidates on write~>uses write-through caching with TTL~~}[^ct-3] for consistency.

## Accepted With No Discussion (Clean Compact Candidate)

The logging pipeline {++ships logs to a centralized aggregator via structured JSON over TCP++}[^ct-4] for analysis.

[^ct-1]: @alice | 2026-03-08 | ins | accepted
...
[^ct-4]: @alice | 2026-03-08 | ins | accepted
