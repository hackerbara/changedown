...
The service mesh {++routes traffic through an Envoy sidecar proxy++}[^cn-1] for load balancing.

## Bob's Proposed Change

The deployment pipeline {++includes canary releases with automated rollback++}[^cn-2] for safety.

## Already Accepted Change

The monitoring stack uses {++Prometheus with Grafana dashboards for real-time alerting++}[^cn-3] in production.

## Already Rejected Deletion

The feature flag system {--will be replaced with a third-party solution--}[^cn-4] for simplicity.
