...
The service {++implements circuit breakers for all downstream dependencies++}[^ct-1] to prevent cascading failures.

## Proposed Substitution

The data serialization uses {~~XML with XSLT transforms~>Protocol Buffers with schema evolution~~}[^ct-2] for efficiency.

## Accepted Change

The health check endpoint {++returns detailed component status including database, cache, and queue connectivity++}[^ct-3] for monitoring.

## Rejected Deletion

The admin dashboard {--requires VPN access for all administrative operations--}[^ct-4] for security.

## Highlighted Text With Comment

{==The SLA guarantees 99.9% uptime for the public API with a 200ms p99 latency target.==}{>>This SLA needs legal review before publishing to customers.<<}[^ct-5]
...
[^ct-5]: @alice | 2026-03-09 | hlt | proposed
