...
The API uses {~~REST~>{~~GraphQL~>GraphQL (note: {++ and {-- are markup delimiters)~~}[^ct-24]~~}[^ct-1] for the public interface.
Authentication uses {~~API keys~>OAuth2 with JWTs (JSON Web Tokens) for enhanced security and auditability~~}[^ct-25]~~}[^ct-2] for all endpoints.
{++Rate limiting is set to 1000 requests per minute.++}[^ct-3]
{--Legacy XML support is {~~maintained~>obsolete~~}[^ct-27].--}[^ct-4]

## {~~Caching~>Cache Management Strategy\n\nThis section covers all caching mechanisms used throughout the platform, including edge, application, and distributed caches.~~}[^ct-26]

{~~The system uses {~~no caching~>Redis caching with 5-minute TTL~~}[^ct-5] for performance.~>The system uses Redis caching with 5-minute TTL and LRU eviction for performance.~~}[^ct-28]~>Redis caching with 5-minute TTL~~}[^ct-5] for performance.
...
[^ct-24]: @ai:claude-haiku-4-5 | 2026-02-20 | sub | proposed
...
[^ct-25]: @ai:claude-haiku-4-5 | 2026-02-20 | sub | proposed
...
[^ct-26]: @ai:claude-haiku-4-5 | 2026-02-20 | sub | proposed
...
[^ct-27]: @ai:claude-haiku-4-5 | 2026-02-20 | sub | proposed
    @ai:claude-haiku-4-5 2026-02-20 [suggestion]: Consider refactoring this

[^ct-28]: @ai:claude-sonnet-4-6 | 2026-02-20 | sub | proposed
