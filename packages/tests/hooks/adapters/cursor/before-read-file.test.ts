...
import { handleBeforeReadFile } from 'changetracks-hooks/internals';
...
describe('Cursor beforeReadFile handler', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ct-cursor-read-'));
  ...
  it('uses workspace_roots[0] as projectDir when cwd is absent', async () => {
    ...
    const result = await handleBeforeReadFile(input);
    // Should find config in tmpDir/.changetracks/ and evaluate properly
    expect(result.continue).toBe(true);
    expect(result.permission).toBe('allow');
  });
});
