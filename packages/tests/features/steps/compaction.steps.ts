...
Given('an L3 document with 2 accepted and 1 proposed change', function (this: ChangeDownWorld) {
  this.l3Text = [
    ...
    '[^cn-2]: @bob | 2026-03-20 | ins | accepted',
    '    1:b2 The quick beautiful brown fox jumps {++happily ++}over the lazy dog.',
    '[^cn-3]: @carol | 2026-03-21 | ins | proposed',
    '    1:c3 The quick beautiful brown fox jumps happily over the {++lazy ++}dog.',
  ].join('\n');
});

Given('an L3 document with 1 proposed insertion', function (this: ChangeDownWorld) {
  this.l3Text = [
...
Given('an L3 document with a supersede chain cn-1 to cn-2', function (this: ChangeDownWorld) {
  this.l3Text = [
...
Given('an L3 document that has already been compacted once', async function (this: ChangeDownWorld) {
...
Given('new decided changes exist after the first compaction', function (this: ChangeDownWorld) {
  ...
  const newFootnote = [
    '[^cn-3]: @bob | 2026-03-21 | ins | accepted',
...
Given('an L3 document with 1 accepted change', function (this: ChangeDownWorld) {
  this.l3Text = [
    'Some text here.',
    '',
    '[^cn-1]: @alice | 2026-03-20 | ins | accepted',
...
When('I compact all decided changes', async function (this: ChangeDownWorld) {
...
When('I compact with undecided policy {string}', async function (this: ChangeDownWorld, policy: string) {
...
When('I compact cn-2', async function (this: ChangeDownWorld) {
  this.compactionResult = await compact(this.l3Text, {
    targets: ['cn-2'],
    undecidedPolicy: 'accept',
  });
...
When('I compact the new decided changes', async function (this: ChangeDownWorld) {
  this.compactionResult = await compact(this.l3Text, {
    targets: 'all-decided',
    undecidedPolicy: 'accept',
  });
...
When(
  ...
  async function (this: ChangeDownWorld, note: string) {
    this.compactionResult = await compact(this.l3Text, {
      targets: 'all-decided',
      undecidedPolicy: 'accept',
      boundaryMeta: { note },
...
Then('the decided footnotes are removed', function (this: ChangeDownWorld) {
  const result = this.compactionResult!;
  assert.ok(result, 'compactionResult must exist');
  assert.ok(!result.text.includes('[^cn-1]:'), 'cn-1 footnote must be removed');
  assert.ok(!result.text.includes('[^cn-2]:'), 'cn-2 footnote must be removed');
});

Then('the proposed footnote is preserved', function (this: ChangeDownWorld) {
  const result = this.compactionResult!;
  assert.ok(result.text.includes('[^cn-3]:'), 'cn-3 proposed footnote must be preserved');
});

Then('a compaction-boundary footnote exists', function (this: ChangeDownWorld) {
  const result = this.compactionResult!;
  assert.ok(result.text.includes('compaction-boundary'), 'compaction-boundary must exist');
});

Then('the insertion text remains in the body', function (this: ChangeDownWorld) {
  const result = this.compactionResult!;
  assert.ok(result.text.includes('beautiful'), 'inserted text "beautiful" must remain in body');
});

Then('the footnote is removed', function (this: ChangeDownWorld) {
  const result = this.compactionResult!;
  assert.ok(!result.text.includes('[^cn-1]:'), 'cn-1 footnote must be removed');
});

Then('the insertion text is removed from the body', function (this: ChangeDownWorld) {
  const result = this.compactionResult!;
...
Then('both cn-1 and cn-2 are compacted', function (this: ChangeDownWorld) {
  const result = this.compactionResult!;
  assert.ok(result.compactedIds.includes('cn-1'), 'cn-1 must be in compactedIds');
  assert.ok(result.compactedIds.includes('cn-2'), 'cn-2 must be in compactedIds');
  assert.ok(!result.text.includes('[^cn-1]:'), 'cn-1 footnote must be removed');
  assert.ok(!result.text.includes('[^cn-2]:'), 'cn-2 footnote must be removed');
});

Then('the compacted file is self-sufficient', function (this: ChangeDownWorld) {
  const result = this.compactionResult!;
...
Then('two compaction-boundary footnotes exist', function (this: ChangeDownWorld) {
  const result = this.compactionResult!;
...
Then('the boundary footnote contains {string}', function (this: ChangeDownWorld, expected: string) {
  const result = this.compactionResult!;
