...
Connection establishment requires a single round trip (1-RTT) for new connections and zero round trips (0-RTT) for resumed connections. The 0-RTT mode uses previously cached cryptographic parameters, reducing connection latency from 10-20 milliseconds to effectively zero for returning clients.
...
| Page load (3G, 300 ms RTT) | 4.2s | 3.1s | 26% faster |
| Page load (LTE, 50 ms RTT) | 1.1s | 0.95s | 14% faster |
...
QUIC integrates encryption at the transport layer using TLS 1.3. All data in transit is encrypted — there is no plaintext QUIC. The handshake negotiates a shared secret using X25519 and derives traffic keys using HKDF-SHA256.
...
QUIC connections survive IP address changes (e.g., Wi-Fi to cellular handoff). This is a security-sensitive feature because it allows a connection to continue from a different network address without re-authentication.
...
- RTT variations across regions (20 ms intra-region to 200 ms cross-region)
...
| `quic_handshake_duration_ms` | `type={initial,0rtt,retry}` | P99 > 100 ms |
