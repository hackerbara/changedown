...
await esbuild.build({
  ...shared,
  entryPoints: ['src/lab-diagnostics.ts'],
  outfile: 'dist/lab-diagnostics.js',
});
...
// Bridge module entry points
await esbuild.build({
  ...shared,
  entryPoints: ['src/bridge/session-registry.ts'],
  outfile: 'dist/bridge/session-registry.js',
});

await esbuild.build({
  ...shared,
  entryPoints: ['src/bridge/idle-reaper.ts'],
  outfile: 'dist/bridge/idle-reaper.js',
});

// Transport entry points (leader election + HTTP transport modules)
await esbuild.build({
  ...shared,
  entryPoints: ['src/transport/bridge-autospawn.ts'],
  outfile: 'dist/transport/bridge-autospawn.js',
});
...
await esbuild.build({
  ...shared,
  entryPoints: ['src/transport/os-shim.ts'],
  outfile: 'dist/transport/os-shim.js',
});

await esbuild.build({
  ...shared,
  entryPoints: ['src/transport/signals.ts'],
  outfile: 'dist/transport/signals.js',
});
...
await esbuild.build({
  ...shared,
  entryPoints: ['src/transport/word-forwarder.ts'],
  outfile: 'dist/transport/word-forwarder.js',
});
...
// Launcher — CLI escape-hatch for starting the MCP server from the host agent process
await esbuild.build({
  ...shared,
...
console.log('Bundled mcp-server → dist/index.js, dist/cli.js, dist/internals.js, dist/lab-diagnostics.js, dist/remote-worker.js, dist/bridge/session-registry.js, dist/transport/bridge-autospawn.js, dist/transport/fixed-port-leader.js, dist/transport/os-shim.js, dist/transport/signals.js, dist/transport/streamable-http.js, dist/transport/pane-endpoint.js, dist/transport/word-forwarder.js, dist/remote-backend.js, dist/pane-registration.js, dist/launcher.js');
