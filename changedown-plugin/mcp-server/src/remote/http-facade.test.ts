    ...
    room: {
      async callBackendOperation(operation, metadata) {
        const lowered = operation.operation.kind === 'applyChange'
          ...
          : { name: operation.operation.kind, args: 'options' in operation.operation ? operation.operation.options ?? {} : {}, idempotencyKey: metadata.idempotencyKey };
        calls.push(lowered);
...
describe('remote HTTP facade', () => {
  ...
  it('uses the production MCP path to preserve remote Word schema and author synthesis', async () => {
    ...
    expect(toolsBody.tools.map((t: { name: string }) => t.name)).toEqual(['read_tracked_file', 'list_changes', 'propose_change', 'review_changes', 'amend_change', 'supersede_change', 'resolve_thread']);
