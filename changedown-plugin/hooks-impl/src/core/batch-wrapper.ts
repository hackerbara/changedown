...
import { generateFootnoteDefinition, scanMaxCnId, insertTrackingHeader, nowTimestamp } from '@changedown/core';
import { readPendingEdits, clearSessionEdits, type PendingEdit } from '../pending.js';
 ...
 */
function isCreationEdit(edit: PendingEdit, fileContent: string): boolean {
...
export async function applyPendingEdits(
  ...
  config: {
    ...
    policy: { mode: string; creation_tracking: CreationTracking };
  },
): Promise<BatchResult> {
  const emptyResult: BatchResult = { editsApplied: 0, changeIds: [], message: '' };
  ...
  // Pre-allocate IDs in forward order, then reverse for offset-safe processing
  type EditWithId = PendingEdit & { changeId: string };
  const allEditsWithIds: EditWithId[] = [];
  let childCounter = 0;
  ...
  const editsWithIdsByFile = new Map<string, EditWithId[]>();
  ...
  const message =
    `ChangeDown recorded ${totalEdits} edit(s) this turn:\n${summary}\n` +
