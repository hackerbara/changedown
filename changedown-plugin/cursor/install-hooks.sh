#!/usr/bin/env bash
# Install ChangeDown hooks for Cursor
set -euo pipefail
...
# Template uses portable relative paths (changedown-plugin/...);
# installed copy gets absolute paths so hooks work regardless of cwd.
sed "s|changedown-plugin/|${PLUGIN_DIR}/|g" "$SCRIPT_DIR/hooks.json" > "$PROJECT_DIR/.cursor/hooks.json"
...
echo "  3. Install SKILL.md if not done: ./changedown-plugin/cursor/install-skill.sh"
